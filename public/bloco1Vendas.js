...
/* ==========================================================================
   ...
   ========================================================================== */
const GeradorOS = {
  async gerarPDF(dados, acao = 'download') {
    ...
    const perfil = perfilHelper ? perfilHelper.obter() : {
      ...
      slogan: 'Serviços Elétricos de Alta Qualidade',
    ...
    if (servicos.length === 0) {
      linhasServicos.push([{ text: 'Serviço elétrico conforme vistoria técnica prévia.', colSpan: 4, italics: true, fontSize: 7 }, {}, {}, {}]);
    } else {
      servicos.forEach(s => {
        const pUnit = Number(s.precoUnit || s.valor || 0);
        const q = Number(s.qtd || 1);
        linhasServicos.push([
          { text: s.descricao || s.item || 'Serviço Técnico', fontSize: 7 },
    ...
    if (materiais.length === 0) {
      ...
      materiais.forEach(m => {
        ...
        linhasMateriais.push([
          { text: m.descricao || m.item || 'Insumo Elétrico', fontSize: 7 },
          { text: String(q), fontSize: 7, alignment: 'center' },
          { text: pUnit.toLocaleString('pt-MZ', { minimumFractionDigits: 2 }), fontSize: 7, alignment: 'right' },
          { text: (q * pUnit).toLocaleString('pt-MZ', { minimumFractionDigits: 2 }), fontSize: 7, alignment: 'right' }
        ]);
    ...
    if (tipoDoc === 'RECIBO') {
      // Carimbo PAGO de alto impacto visual
    ...
    if (tipoDoc === 'CONTRATO') {
      // Cláusulas Contratuais Sintéticas para 1 página
      conteudoDinamico.push(
        {
          stack: [
            ...
            { text: `2. VALOR E FORMA DE PAGAMENTO: O valor total acordado é de ${totalGeral.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })} MZN, pago conforme estipulado neste termo (${dados.condicoesPagamento || '50% entrada e 50% na conclusão'}).`, fontSize: 6.8, color: '#334155' },
            { text: `3. PRAZO E GARANTIA: O prazo estimado é de ${dados.prazoExecucao || '3 dias úteis'}. Garantia de ${dados.garantiaDias || 90} dias sobre a mão de obra contra defeitos de montagem.`, fontSize: 6.8, color: '#334155' }
          ],
          margin: [0, 0, 0, 6]
        }
      );
          ...
          columns: [
            {
              width: '*',
              stack: [
                { text: 'CONDIÇÕES & OBSERVAÇÕES:', fontSize: 7.5, bold: true, color: '#0f172a' },
                { text: `• Pagamento: ${dados.condicoesPagamento || 'À vista ou 50% na aprovação e saldo no encerramento.'}`, fontSize: 7, color: '#334155' },
                { text: `• Prazo de Execução: ${dados.prazoExecucao || 'Imediato / Conforme cronograma'}`, fontSize: 7, color: '#334155' },
                { text: `• Garantia da Mão de Obra: ${dados.garantiaDias || 90} dias`, fontSize: 7, color: '#334155' },
                dados.observacoes ? { text: `• Nota: ${dados.observacoes}`, fontSize: 6.8, italics: true, color: '#475569' } : {}
              ]
            },
            {
              width: 175,
...
const ListaMateriaisAutomatica = {
  ...
  async gerarPDF(dadosObra, comodos = [], acao = 'download') {
    await carregarPdfMakeSeNecessario();
    const perfilHelper = (typeof window !== 'undefined' && window.PerfilTecnico) ? window.PerfilTecnico : null;
    const perfil = perfilHelper ? perfilHelper.obter() : { nome: 'Profissional Técnico', slogan: 'Serviços Especializados' };

    const quant = this.calcularPorComodos(comodos);
    const cabecalhoPDF = perfilHelper ? perfilHelper.gerarCabecalhoPDF(
      'LISTA QUANTITATIVA DE MATERIAIS ELÉTRICOS',
      `Obra: ${dadosObra.nome || 'Instalação Residencial'} | Data: ${new Date().toLocaleDateString('pt-MZ')}`
    ) : [];
    ...
    const linhasTabela = [
      [
        ...
        { text: 'Quantidade Recomendada', bold: true, fillColor: '#0f172a', color: '#ffffff', fontSize: 7.5, alignment: 'center' }
      ],
      [{ text: 'Condutor Flexível 1.5mm²', fontSize: 7 }, { text: 'Cobre antichama 750V (Iluminação)', fontSize: 7 }, { text: `${quant.cabo1_5} metros`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Condutor Flexível 2.5mm²', fontSize: 7 }, { text: 'Cobre antichama 750V (Tomadas TUG)', fontSize: 7 }, { text: `${quant.cabo2_5} metros`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Condutor Flexível 4.0mm²', fontSize: 7 }, { text: 'Cobre antichama 750V (Cargas Pesadas TUE)', fontSize: 7 }, { text: `${quant.cabo4_0} metros`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Condutor Entrada 6.0mm²', fontSize: 7 }, { text: 'Alimentador do Quadro de Distribuição', fontSize: 7 }, { text: `${quant.caboAlim} metros`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Eletroduto Corrugado 20mm/25mm', fontSize: 7 }, { text: 'Mangueira amarela / laranja normatizada', fontSize: 7 }, { text: `${quant.eletrodutos} metros`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Disjuntores Monopolares 10A DIN', fontSize: 7 }, { text: 'Curva C 3kA (Circuitos de Luz)', fontSize: 7 }, { text: `${quant.disjuntor10A} unidades`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Disjuntores Monopolares 16A DIN', fontSize: 7 }, { text: 'Curva C 3kA (Tomadas de Uso Geral)', fontSize: 7 }, { text: `${quant.disjuntor16A} unidades`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Disjuntores Bipolares 20A/25A DIN', fontSize: 7 }, { text: 'Curva C (AC / Chuveiro / Termoacumulador)', fontSize: 7 }, { text: `${quant.disjuntor20A} unidades`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Disjuntor Geral Bipolar 40A', fontSize: 7 }, { text: 'Entrada Geral da Habitação', fontSize: 7 }, { text: '1 unidade', bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Interruptor Diferencial IDR 40A 30mA', fontSize: 7 }, { text: 'Proteção contra choques elétricos', fontSize: 7 }, { text: '1 unidade', bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Protetor de Surto DPS 20kA 275V', fontSize: 7 }, { text: 'Proteção contra descargas atmosféricas', fontSize: 7 }, { text: '2 unidades', bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Módulos Tomadas 10A + Espelhos', fontSize: 7 }, { text: 'Padronizadas 2P+T 250V', fontSize: 7 }, { text: `${quant.tomadas10A} conjuntos`, bold: true, fontSize: 7, alignment: 'center' }],
      [{ text: 'Módulos Interruptores + Espelhos', fontSize: 7 }, { text: 'Simples / Paralelos 10A', fontSize: 7 }, { text: `${quant.interruptores} conjuntos`, bold: true, fontSize: 7, alignment: 'center' }],
    ...
    const docDef = {
      ...
      content: [
        ...cabecalhoPDF,
        {
          text: `LISTA QUANTITATIVA ESTIMADA COM BASE EM ${comodos.length} CÔMODO(S) LEVANTADO(S)`,
          fontSize: 8,
          bold: true,
          color: '#0369a1',
          margin: [0, 0, 0, 4]
        ...
        {
          text: `Documento de levantamento elaborado pelo profissional: ${perfil.nome}. As quantidades incluem folga técnica estimada de 15%.`,
          fontSize: 6.8,
          italics: true,
          color: '#64748b',
          alignment: 'center'
    ...
    if (acao === 'download') {
      pdf.download(`Lista_Materiais_${(dadosObra.nome || 'Obra').replace(/\s+/g, '_')}.pdf`);
    } else {
      pdf.open();
    }
  }
