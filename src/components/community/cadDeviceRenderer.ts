...
export function renderDevice(
  ...
  // 1. Sombra Projetada 3D da Caixa Termoplástica
  ctx.fillStyle = 'rgba(0, 0, 0, 0.55)';
  ctx.beginPath();
  ctx.roundRect(-cw / 2 + 4 * zoom, -ch / 2 + 6 * zoom, cw, ch, rad);
  ctx.fill();

  // 2. Presilhas DIN 35mm (Trilho Traseiro Superior e Inferior com trava de mola)
  ...
  // Trava metálica ajustável amarela/cinza do engate DIN inferior
  ctx.fillStyle = '#d97706';
  ctx.beginPath();
  ctx.roundRect(-cw * 0.08, ch / 2 + 1 * zoom, cw * 0.16, 3 * zoom, 0.5 * zoom);
  ctx.fill();

  // 3. Corpo Principal — Gradiente Policarbonato Texturizado (RAL 7035)
  ...
  // 4. Borda de Chanfro Tridimensional (High-Gloss Beveling)
  ...
  // 7. Renderização do Módulo Interno Especificado
  ...
  // 8. Parafusos Metálicos Níquel/Aço Inox com Ranhuras Combinadas (+/-)
  renderMetallicScrewTerminals(ctx, c, d, cw, ch, zoom);

  // 9. Simulação Física de Faíscas, Arcos Elétricos e Sobreaquecimento
...
// ----------------------------------------------------------------------------
// 5. SUB-RENDERIZADORES AUXILIARES
// ----------------------------------------------------------------------------

/**
 * Renderiza Cargas 3D Realistas de Alta Potência com Animações
 */
function renderRealisticLoad(
  ...
  ctx.beginPath();
  ctx.roundRect(-boxW / 2, boxY, boxW, boxH, 4 * zoom);
  ctx.fill();
  ctx.stroke();

  // Display LED e LEDs Status
  ctx.fillStyle = isEnergized ? '#0284c7' : '#1e293b';
  ctx.beginPath();
  ctx.arc(-boxW / 3, boxY + boxH / 2, 3 * zoom, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.fillStyle = isEnergized ? '#f0f9ff' : '#475569';
  ctx.font = `bold ${Math.max(8, 10 * zoom)}px monospace`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(d.icon || '⚡', 0, boxY + boxH / 2);

  ctx.restore();
}

/**
 * Renderiza Bornes Com Parafusos Metálicos (+/-) e Profundidade 3D
 */
function renderMetallicScrewTerminals(
  ...
  const termCount = d.terminals ? d.terminals.length : 2;
  const startX = -cw / 2 + cw / (termCount + 1);
  const stepX = cw / (termCount + 1);

  ctx.save();
  for (let i = 0; i < termCount; i++) {
    const tx = startX + i * stepX;
    
    // Terminal Superior e Inferior
    [ -ch / 2 + 7 * zoom, ch / 2 - 7 * zoom ].forEach((ty) => {
      const r = 4.5 * zoom;
      ...
      // Cabeça de metal Aço Cromo Niquelado
      const screwGrad = ctx.createRadialGradient(tx - r * 0.3, ty - r * 0.3, 0.5 * zoom, tx, ty, r);
      screwGrad.addColorStop(0, '#ffffff');
      screwGrad.addColorStop(0.4, '#cbd5e1');
      screwGrad.addColorStop(0.8, '#475569');
      screwGrad.addColorStop(1, '#0f172a');

      ctx.fillStyle = screwGrad;
      ctx.beginPath();
      ctx.arc(tx, ty, r - 1 * zoom, 0, Math.PI * 2);
      ctx.fill();

      // Ranhura Fenda Cross / Pozidriv (+ / -)
      ctx.strokeStyle = '#1e293b';
      ctx.lineWidth = 1 * zoom;
      ctx.beginPath();
      ctx.moveTo(tx - r * 0.5, ty);
      ctx.lineTo(tx + r * 0.5, ty);
...
/**
 * Renderiza Aterramento e Caixas de Junção WAGO
 */
...
/**
 * Renderiza Efeitos Visuais de Falhas (Arcos Elétricos, Fumaça e Faíscas)
 */
function renderFaultVisualEffects(
  ...
  if (!st.sparking && !st.fault && !st.thermal) return;

  ctx.save();
  if (st.sparking || st.fault) {
    // Efeito de Arco Elétrico / Faíscas
    const sparkCount = 6;
    ...
    for (let i = 0; i < sparkCount; i++) {
      const rx = (Math.random() - 0.5) * cw * 0.8;
      const ry = (Math.random() - 0.5) * ch * 0.8;
      
      ctx.beginPath();
      ctx.arc(rx, ry, (1.5 + Math.random() * 2.5) * zoom, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ctx.restore();
